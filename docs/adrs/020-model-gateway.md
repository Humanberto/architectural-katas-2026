...
degrades the estate to a state it already had, not to nothing (R14).
...
passed 021's gate. Fast to flip; not fast to flip *unsafely*.

**A candidate is evaluated offline against 021's golden set first.** If it passes, it
...
(C8) it usually will not be needed, so it is available but not mandatory.
...
provides one.** This is the record 011's third rule requires, and it is what
...
given C13 — nobody here has to build request routing from scratch. Rejected because it
substitutes one provider dependency for another: R14 asks us to survive *a* provider
...
Rejected outright by R14 and C11 — this is the scenario the brief specifically asks us
...
- A provider price rise, model deprecation, or outage is a configuration change at the
  gateway, not an incident response across the codebase (R14).
- Business logic — the concierge's dialogue flow, the copilot's UI — never imports a
  vendor SDK, so 010's module-boundary discipline extends cleanly into the AI surface.
- One log schema serves evaluation, drift detection and cost tracking, so B is building
  one piece of infrastructure rather than three.
- Every fallback bottoms out on the estate with no provider dependency, which is the
  same guarantee 011 already gives Tier 1 and 2, now extended to what happens when
  Tier 3 fails.
...
- Canary and shadow evaluation add latency to adopting a genuinely better model — the
  48-hour minimum bake is a deliberate trade of speed for the verification R13 demands.
- Config-driven prompts and model choice mean a bad config change is possible without a
  code review catching it in the usual place; this is why 021 requires the gate to run
  on every version change, not just code changes.

### Assumptions

- Two generative capabilities is the right scale for a hand-rolled gateway. If D's
  range grows a third, this record should be revisited against buying a router product.
