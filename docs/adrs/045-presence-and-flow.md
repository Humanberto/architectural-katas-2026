...
- No idea which parts of the estate are popular, so investment and staffing are guesswork (P2)
- Must measure area popularity (R3) and act on it (R4)
- 5,000 visitors a day now, 15,000 expected within three years (C8)
- Ticket and family pass purchase is the only transaction in the brief (R1). No food or retail
- The animal collection was private and is opening to the public (P7). We infer from this that
...
| What we measure | How | Certainty | Tier (ADR-011) |
|---|---|---|---|
| Daily entries | Ticket validation, ADR-012 | Fact | 1 |
...
Counters send a few bytes and run on battery, so they use LoRaWAN (ADR-041).
...
counter. Same pattern as ADR-046: frequent cheap estimate, corrected by a scarcer reliable
figure.
...
feeding and enclosure conditions (ADR-047) makes it possible to test later whether species
...
| Decision | Cost |
|---|---|
| Equipment that cannot identify | No question needing the same visitor twice, including returning visitors (R10) |
...
| One independent check | Thinner than ADR-046, which has a keeper's count as a second source |
...
| Cameras with people counting | Puts a camera on every visitor. Cameras stay at enclosures (ADR-046) and that boundary is part of the decision |
| Visitor mobile app | Needs a smartphone and a download, depends on patchy wifi (C1), and holds worse data than anything above |
...
- R10 belongs to the retention seat, and this record makes it harder, not easier
...
someone else. The result is a path with nothing attached to it. This does not recover R10 —
...
cards reporting through the day would exceed the airtime and gateway limits ADR-041 depends
...
| Additional hardware | Well outside the MQTT budget (C3), so needs the C14 argument |
...
independent check, consumable with no structural change: same topic tree (ADR-042), same clock
(ADR-047).
...
- **ADR-011** tiers used here
- **ADR-012** ticket validation, used for calibration
- **ADR-041** carries the counters, and explains why cards would not use LoRaWAN
- **ADR-046** same estimate-and-correct pattern, applied to animals
- **ADR-047** records visitor counts alongside weather and feeding
