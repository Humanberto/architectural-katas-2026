...
**Every capability generalises the disposition pattern 043 already sets for welfare
...
(a keeper checking an alert, an engineer reading a ride advisory). For Tier 3, the
...
against zone counts (012 vs 042/043's range), a stock ledger against the piranha
population estimate. This needs no golden set and no human disposition — it is the
same comparison 021's Tier 2 gate makes, just running forever instead of once.
...
compared to the rate the capability had when it passed its 021 gate. A rise of more
...
- Tier 3: the gateway automatically reverts the affected capability to its last
  ...
  because, unlike Tier 1/2, there is no vet-set limit or physical ledger standing
...
**Canary bake-in, set here and referenced by 020**: a new Tier 3 version serves 10% of
...
Given C13, this is deliberately the only surface — a generalist can read seven lines
...
**B. Full observability/APM tooling.** Rejected by C13 directly — it is built for
...
- Detection cost is close to zero for Tier 1/2, because it reuses work a person was
  already doing (043's pattern, generalised).
...
- One digest, one recipient role, no dashboard to build or forget — matches C13.
- The same disposition log that detects drift here is the evidence 021's audit-rate
...
- The operations-lead role from 020 is checked at least weekly; a digest nobody opens is
